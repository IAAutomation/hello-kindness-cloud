  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[color:var(--color-cream)] text-[color:var(--color-espresso)]">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-0">
        ...
        <div className="absolute top-1/3 -right-32 h-[460px] w-[460px] rounded-full bg-[color:var(--color-apricot)]/25 blur-3xl animate-blob" style={{ animationDelay: "3s" }} />
        <div className="absolute bottom-0 left-1/3 h-[380px] w-[380px] rounded-full bg-[color:var(--color-butter)]/50 blur-3xl animate-blob" style={{ animationDelay: "6s" }} />
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-6 py-16 md:py-24">
        ...
        <section className="mt-24">
          <h2 className="mb-8 text-center font-display italic-serif text-4xl">What's in the zip</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              >
                ...
                <div className="font-medium text-sm text-[color:var(--color-espresso)]">{c.t}</div>
                <div className="text-xs text-[color:var(--color-muted-ink)] mt-0.5">{c.s}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mt-24">
          <div className="mx-auto max-w-3xl rounded-3xl border border-[color:var(--color-gold)]/30 bg-white/70 p-8 md:p-12 backdrop-blur-md shadow-[0_30px_80px_-40px_rgba(27,26,23,0.25)]">
            <div className="flex items-center gap-3">
              <span className="text-xs uppercase tracking-[0.24em] text-[color:var(--color-gold)]">Install</span>
              <span className="h-px flex-1 bg-gradient-to-r from-[color:var(--color-gold)]/40 to-transparent" />
            </div>
            <h2 className="mt-3 font-display italic-serif text-4xl md:text-5xl">Five taps. Done.</h2>

            <ol className="mt-10 space-y-6">
              {[
                { t: "Download & unzip", s: "Extract unlimitly-extension.zip anywhere on your machine." },
                { t: "Open chrome://extensions", s: "Paste into your address bar (works in Chrome, Edge, Brave, Arc, Opera)." },
                { t: "Enable Developer mode", s: "Toggle in the top-right corner." },
                { t: "Click 'Load unpacked'", s: "Select the unzipped folder — that's the whole install." },
                { t: "Open lovable.dev", s: "Unlimitly's side panel appears in the toolbar. Sign in and go." },
              ].map((step, i) => (
                <li key={i} className="group flex gap-5 items-start">
                  ...
                  <div className="pt-1.5">
                    <div className="text-base font-medium text-[color:var(--color-espresso)]">{step.t}</div>
                    <div className="text-sm text-[color:var(--color-muted-ink)] mt-1 leading-relaxed">{step.s}</div>
                  </div>
                </li>
              ))}
            </ol>
